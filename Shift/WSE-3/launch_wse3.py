...
def main():
    ...
    with open(f"{out_path}/artifact_{P}_{L}_{Mt}_{Nt}.json", "r", encoding="utf-8") as f:
        data = json.load(f)
    ...
    with SdkRuntime(artifact_id, simulator=args.simulator) as runner:
        ...
        runner.memcpy_d2h(res3_1d_u32, sym_res, L, 0, P, P, Mt*Nt, \
                        streaming=False, data_type=io_dtype, order=memcpy_order, nonblock=False)
        res3_1d_fp16 = sdk_utils.memcpy_view(res3_1d_u32, np.dtype(np.float16))
        res3 = res3_1d_fp16.reshape((P, P, Mt, Nt))
    ...
    min_src_time_start = src_time_start.min()
    max_des_time_end = des_time_end.max()
    ...
    freq_ghz = 1.1
    time_cost = (max_des_time_end - min_src_time_start) / total_repeat_times / (freq_ghz*1e6)
